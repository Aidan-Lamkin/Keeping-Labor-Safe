CREATE PROCEDURE ana.uspSelectAllRiskFactorsByCaseID
	@CaseID INT 
AS 
	...
	WHERE m_IsEnabled = 1
		...
		AND DataType <> 'DATETIME2(0)'
	SELECT @Query += ' ELSE 0 END) AS FetalRF FROM dbo.vwAllRiskFactors arf LEFT JOIN ana.tKlsFrame10Min frm ON arf.CaseID = frm.CaseID WHERE arf.CaseID =' + CAST(@CaseID AS VARCHAR(128)) + ' GROUP BY arf.CaseID, FrameStartDateTime, FrameEndDateTime '
